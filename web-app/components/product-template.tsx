                    ...
                    <div>
                        ...
                        <div className="flex justify-between items-center gap-2">
                            {product_size_variants.map((variant) => {
                                return (
                                    <button type="button" key={variant.id} className="text-sm font-medium h-10 rounded-md p-2 flex-1 hover:shadow-lg transition-shadow ease-in-out duration-150 border hover:bg-cyan-100 hover:border-cyan-600">
                                        {variant.name}
                                    </button>
                                )
                            })}
    ...
    return (<div className="w-full">
        <div className="text-xs font-semibold py-8">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                {Object.entries(product.specifications as unknown as Record<string, string>).map(([key, value]: [string, unknown]) => {
                    return (
                        <div key={key}>
                            <span className="font-semibold capitalize">{key}</span>
                            <p className="text-gray-600">
                                {value === null || value === "unknown" ? "-" : value as string}
                            </p>
                        </div>
                    )
                })}
